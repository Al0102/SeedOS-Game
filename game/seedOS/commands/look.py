...
def get_look_command():
    """
    Get the dictionary data for the look command.

    :postcondition: get the data for the look command
    :return: a dictionary representing the data for the look command
    """
...
def run_look(seed_system, tokens):
    """
    ...
    :postcondition: look through the content of a text file
    :return: a tuple of 2 strings representing the success status and a status message
    """
    status = "success"
    if len(tokens) != 1:
        status = "argument_error"
        status_message = f"|'look' expects exactly 1 argument: [path]|\n{len(tokens)} != 1"
    else:
        ...
        try:
            ...
            if path_data["type"] != "file" or path_data["extension"] != "txt":
                status = "argument_error"
                status_message = f"|Can't read non-txt files|\n{style(new_path, 'underline')}"
