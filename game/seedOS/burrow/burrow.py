...
from game.utilities import targets_have_key, targets_with_key, remove_escape_codes, sum_vectors
...
def load_board_from_file(board_file: TextIO):
    ...
    while line := board_file.readline():
        ...
        for column, character in enumerate(line[first_character_index:last_character_index], first_character_index):
            try:
                spawn_entity(board, entity=get_entity_types()[entity_icons[character]], position=(column + 1, row))
            except KeyError:
...
def get_entity_types():
    ...
    entities = (
        ...
        create_entity(
            "large_corruption", style("@", "red"),
            base_damage=3, solid=False, type="corruption"),
        create_entity(
            "nectar", style("n", "yellow"),
...
def spawn_entity(board: dict, position: tuple, entity=None) -> dict | None:
    """
    ...
    :param entity: (default None) a dictionary representing the data of the entity to spawn
    ...
    >>> spawn_entity(new_board, (1, 1), {'name': 'wall', 'icon': '#', 'health': 5, 'position': (1, 1), 'id': 0}) == {
    ...
    try:
        ...
        print(f"Expected <entity> to be passed, but neither were found", file=stderr)
    else:
...
def remove_entity(board: dict, entity: dict) -> None:
    """
    Remove an entity from the board.

    :param board: a dictionary with position keys and list values representing the board.
    :param entity: a dictionary representing the data of the entity to remove
    :precondition: board must be a dictionary of form <position tuple>: <entity dictionary>
    :precondition: entity must be a dictionary of entity data with at least these key-value-pairs:
                       "id": <int> and "position": <position tuple>
    :precondition: entity must exist within the board already
    :postcondition: remove <entity> from <board>
    :postcondition: modifies the original board dictionary
    >>> new_entity = {"id": 0, "position": (1, 1)}
    >>> new_board = {(1, 1): [new_entity]}
    >>> remove_entity(new_board, new_entity)
    >>> new_board == {(1, 1): []}
    True
    """
    board[entity["position"]].remove(entity)
